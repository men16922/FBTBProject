      ...
      <div class="row">
        ...
        <div class="row">
            ...
            <div class="col" style="background-color:white">
                ...
                <div class="row mt-3">
                    <!--타이틀-->
                    <div class="col-4 mt-3" ><span style="font-family:'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; color: rgb(46, 46, 46); font-weight:bold; font-size: 36px;">공장 관리</span></div>
                    <div class="col"></div>
                    <!---네비게이터-->
                    <div class="col-6">
                        <div class="row">                                   
...
<form action="./factory_view.do" method="post">
      ...
      <option value="">지점선택</option>
         <c:forEach var="cccc" items="${dataList3 }">
   ...
   <div class="row"> <!-- 테이블 -->
            <div class="col">
               <table class="table table-hover shadow-sm p-3 mb-5 bg-white rounded">
                  <thead class="shadow-none p-3 mb-5 bg-light rounded">
                     ...
                     <td>번호</td><td>공장명</td><td>담당자</td><td>주소</td><td>연락처</td><td></td><td></td>
                     </tr>
                  </thead>
                  <tbody>
                  <c:forEach var="aaaa" items="${boardList_factory }" varStatus="xxx">
                     <tr style="text-align:center">
                     <th scope="row">${xxx.index+1 }</th>
                        <td><a href="${pageContext.request.contextPath }/management/read_factory_page.do?branch_no=${aaaa.branchVo.branch_no }&emp_code=${aaaa.branchVo.emp_code }">${aaaa.branchVo.branch_name }</a></td>
                        <td>${aaaa.employeeVo.emp_name }</td>
                        <td>${aaaa.branchVo.branch_address }</td>
                        <td>${aaaa.branchVo.branch_phone }</td>
                        <td><a class="btn btn-outline-primary btn-sm-block" href="factory_modify.do?branch_no=${aaaa.branchVo.branch_no}">수정</a></td>
                        <td>
                        <c:if test="${aaaa.employeeVo.emp_no==0}">
                        <a class="btn btn-primary btn-sm-block" href="factory_modify.do?branch_no=${aaaa.branchVo.branch_no}">담당자 등록</a>
                        </c:if>
                        </td>
                     </tr>
                  </c:forEach>
            ...
            <div class="col"> <!-- 페이지 버튼 -->
               ...
               <ul class="pagination d-inline-flex">
                  ...
                  <li class="page-item<c:if test="${beginPage-1 <= 0}"> active</c:if>">
                   <a class="page-link" href="./factory_view.do?dept_no=${param.dept_no}&branch_no=${param.branch_no}&emp_name=${param.emp_name}&branch_address=${param.branch_address}&curr_page=${beginPage-1 }">이전</a></li>
                    <c:forEach begin="${beginPage }" end="${endPage }" var="i">
                       <li class="page-item<c:if test="${curr_page==i}"> active</c:if>">
                       <a class="page-link" href="./factory_view.do?dept_no=${param.dept_no}&branch_no=${param.branch_no}&emp_name=${param.emp_name}&branch_address=${param.branch_address}&curr_page=${i}">${i}</a></li>
                    </c:forEach>
                   <li class="page-item<c:if test="${endPage+1 >= (totalCount-1)/10+1 }"> active</c:if>">
                   <a class="page-link" href="./factory_view.do?dept_no=${param.dept_no}&branch_no=${param.branch_no}&emp_name=${param.emp_name}&branch_address=${param.branch_address}&curr_page=${endPage+1 }">다음</a></li>
                 <!-- 
            ...
            <a class="btn btn-primary btn-sm-block" href="${pageContext.request.contextPath }/management/factory_add.do">신규 공장 등록</a>
