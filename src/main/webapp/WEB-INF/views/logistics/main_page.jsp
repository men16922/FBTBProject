		...
		<div class="container-fluid">
			...
			<div class="row">
					...
					<ul class="list-group list-group-flush">
						...
						<li class="list-group-item"
							style="height: 105px; background-color: #7393a7; color: white; font-weight: bold;"><h2
								class="mt-3" align="center">물류부게시판</h2></li>
				...
				<div class="col pl-5 pr-5">
						...
						<div class="col" style="background-color: white">
							...
							<div class="row mt-3">
								...
								<div class="col-4 mt-3">
									<span
										style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; color: rgb(46, 46, 46); font-weight: bold; font-size: 36px;">물류부게시판</span>
								</div>
								<div class="col"></div>
								<!---네비게이터-->
								<div class="col-6">
					...
					<form action="${pageContext.request.contextPath }/logistics/main_page.do" method="post">
					<div class="row my-3">
						<div class="col"></div>
						<div class="col-6">
							<div class="row">
													...
													<c:forEach items="${noticelist }" var="data">
														<tr style="font-weight: bold" bgcolor="#e8ecf1">
															<td class="text-center"><c:if
																	test="${data.departmentBoardVo.check_notice eq 'Y'.charAt(0) }">[공지]</c:if>
															</td>
															<td class="text-center">${data.departmentBoardVo.dept_board_no }</td>
															<td class="text-center"><a
																href="${pageContext.request.contextPath }/logistics/board_read_page.do?dept_board_no=${data.departmentBoardVo.dept_board_no }">${data.departmentBoardVo.dept_board_title }</a></td>
															<td class="text-center">${data.branchVo.branch_name }</td>
															<td class="text-center">${data.employeeVo.emp_name }</td>
															<td class="text-center"><fmt:formatDate
																	value="${data.departmentBoardVo.dept_board_writedate }"
									...
									<nav aria-label="Page navigation example">
										<ul class="pagination">
											<li
												...
												class="page-link"
												href="${pageContext.request.contextPath }/logistics/main_page.do?currPage=${beginPage-1 }&searchWord=${param.searchWord}">이전</a></li>
											<c:forEach begin="${beginPage }" end="${endPage }" var="i">
												<!-- 띄어쓰기 주의 -->
												<li
													class="page-item<c:if test="${currPage==i }"> active</c:if>">
													<a class="page-link"
													href="${pageContext.request.contextPath }/logistics/main_page.do?currPage=${i }&searchWord=${param.searchWord}">${i }</a>
												</li>
											</c:forEach>
											<li
												class="page-item<c:if test="${endPage+1 > (totalCount-1)/10+1 }"> disabled</c:if>"><a
												class="page-link"
												href="${pageContext.request.contextPath }/logistics/main_page.do?currPage=${endPage+1 }&searchWord=${param.searchWord}">다음</a></li>
										</ul>
									</nav>
								</div>
								<div class="col-1">
									<a class="btn btn-primary btn-sm btn-block"
										href="${pageContext.request.contextPath }/logistics/board_write_page.do">글쓰기</a>
								</div>
							</div>
