...
<body>
      ...
      <div class="row">
        ...
        <div class="row">
            ...
            <div class="col" style="background-color:white">
                ...
                <div class="row mt-3">
                    <!--타이틀-->
                    <div class="col-4 mt-3" ><span style="font-family:'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; color: rgb(46, 46, 46); font-weight:bold; font-size: 36px;">사원조회</span></div>
                    <div class="col"></div>
                    <!---네비게이터-->
                    <div class="col-6">
                        <div class="row">                                   
                            ...
                            <div class="col mt-5" style="display:inline;">     
                                <ul style="list-style:none; margin-top: 10px; margin-bottom: 5px; margin-left: 100px; text-decoration-color: gray">
                                    <li>
                                        ...
                                        >
                                        <a style="color: gray; font-size: 14px" href="${pageContext.request.contextPath }/management/employee_view.do">사원조회</a>
  ...
  <form action="./employee_view.do" method="get" id="frm">
               ...
               <div class="row mt-5">
                  ...
                  <table class="table bg-light">
                     ...
                     <tr>
                        <th>부서/지점</th>
                        <td>
		<select id="dept_no" name="dept_no" class="form-control form-control-sm" onchange="test5(),sendData()">
                        ...
                        <td>
		<select id="branch_no" name="branch_no" class="form-control form-control-sm" onchange="sendData()">
                     ...
                     <tr>
                        <th>직급/직원명</th>
                        <td>
                   <select id="rank_no" name="rank_no" class="form-control form-control-sm" onchange="sendData()">
                        ...
                        <td><input id="emp_name" placeholder="직원명을 입력해주세요." name="emp_name" type="text" class="form-control form-control-sm" value=${param.emp_name }></td>
                     </tr>
                     

                     <tr>
                        <th class="shadow-none p-3 mb-5 bg-light rounded">사번/입사일</th>
                        <td>
                   <input id="emp_no" placeholder="사원번호를 입력해주세요." name="emp_no" type="text" class="form-control form-control-sm" value=${param.emp_no }>
		  </td>
                        <td><input type="date" name="start_date" id="start_date" class="form-control form-control-sm" value=${param.start_date } onchange="sendData()">	</td>
                     </tr>
	...
	<!-- <div class="col mr-5 pr-5 ml-0">--><div class="col">
	<!-- <div class="col mr-5 pr-5 ml-0">--><div class="col">
		<select id="orderby" name="orderby" class="form-control" onchange="sendData()">
					    <option value="">정렬선택</option>
					    <option value="1" <c:if test="${param.orderby==1}">selected="selected"</c:if>>사원번호순</option>
					    <option value="2" <c:if test="${param.orderby==2}">selected="selected"</c:if>>이름순</option>
					    <option value="3" <c:if test="${param.orderby==3}">selected="selected"</c:if>>부서순</option>
					    <option value="4" <c:if test="${param.orderby==4}">selected="selected"</c:if>>직급순</option>
					    <option value="5" <c:if test="${param.orderby==5}">selected="selected"</c:if>>입사일순</option>				    
		</select>
	</div>
				...
				<div class="col">
					<table class="table table-hover shadow-sm p-3 mb-5 bg-white rounded" id="prolist">
						<thead class="shadow-none p-3 mb-5 bg-light rounded">
							...
							<td>사원번호</td><td>이름</td><td>부서</td><td>직급</td><td>입사일</td>
							</tr>
						</thead>
						<tbody id="tbody">
						<c:forEach var="aaaa" items="${boardList_employee_main }">
							<tr style="text-align:center">
								...
								<td><a href="./employee_confirm.do?emp_no=${aaaa.employeeVo.emp_no }">${aaaa.employeeVo.emp_name }</a></td>
			...
			<div class="row"> <!-- 버튼들... -->
				...
				<div class="col"> <!-- 페이지 버튼 -->
					...
					<ul class="pagination d-inline-flex">
					    ...
					    <a class="ttt page-link" href="./employee_view.do?dept_no=${param.dept_no}&branch_no=${param.branch_no}&rank_no=${param.rank_no}&emp_name=${param.emp_name}&emp_no=${param.emp_no}&start_date=${param.start_date}&curr_page=${beginPage-1 }&orderby=${param.orderby}">이전</a></li>
					  	<c:forEach begin="${beginPage }" end="${endPage }" var="i">
					  		<li class="page-item<c:if test="${curr_page==i}"> active</c:if>">
					  		<a class="ttt page-link" href="./employee_view.do?dept_no=${param.dept_no}&branch_no=${param.branch_no}&rank_no=${param.rank_no}&emp_name=${param.emp_name}&emp_no=${param.emp_no}&start_date=${param.start_date}&curr_page=${i}&orderby=${param.orderby}">${i}</a></li>
					  	</c:forEach>
					    <li class="page-item<c:if test="${endPage+1 >= (totalCount-1)/10+1 }"> active</c:if>">
					    <a class="ttt page-link" href="./employee_view.do?dept_no=${param.dept_no}&branch_no=${param.branch_no}&rank_no=${param.rank_no}&emp_name=${param.emp_name}&emp_no=${param.emp_no}&start_date=${param.start_date}&curr_page=${endPage+1 }&orderby=${param.orderby}">다음</a></li>
					  <!-- 
