...
<script type="text/javascript" charset="UTF-8">
	...
	function sendData(){
		...
		var store_out_item_no = urlParams.get("store_out_item_no");
		var product_name = urlParams.get("product_name");
		var out_start_date = urlParams.get("out_start_date");
		var out_final_date = urlParams.get("out_final_date");
		...
		if(currPage==null)
	         ...
	         if(xmlhttp.readyState==4 && xmlhttp.status == 200){
	          ...
	          var innerHtml="";
	            ...
	            for(var i=0; i<xxx.length; i++){
	         		...
	         		if(store_out_item_no==null)
	         			store_out_item_no="";
	         		x+="&store_out_item_no="+store_out_item_no;
	         		if(product_name==null)
	         			product_name="0";
	         		x+="&product_name="+product_name;
	         		 if(out_start_date==null)
	         			out_start_date="";
	         		x+="&out_start_date="+out_start_date;
	         		 if(out_final_date==null)
	         			out_final_date="";
	         		x+="&out_final_date="+out_final_date;	
	         		 if(emp_name==null)
	         			emp_name="";
	         		x+="&emp_name="+emp_name;	
	         		x+="&sort="+sort;
	      ...
	      var requestUrl = "${pageContext.request.contextPath }/sales/storeout_search_process.do?";
	      if(store_out_item_no==null)
	    	  store_out_item_no="";
	      requestUrl += "store_out_item_no=" + store_out_item_no;
	      if(product_name==null)
	    	  product_name="";
	      requestUrl += "&product_name=" + product_name;
	      if(out_start_date==null)
	    	  out_start_date="";
	      requestUrl += "&out_start_date=" + out_start_date;
	      if(out_final_date==null)
	    	  out_final_date="";
	      requestUrl += "&out_final_date=" + out_final_date;
	      if(emp_name==null)
	    	  emp_name="";
	...
	function searchprice(price){
		const urlParams = new URLSearchParams(window.location.search);
			...
			if(currPage==null)
		      ...
		      var requestUrl = "${pageContext.request.contextPath }/sales/storeout_price_process.do?";
...
<div class="container-fluid">
         ...
         <!--  사이드바 끝   -->
            ...
            <div class="row">
                ...
                <div class="col" style="background-color:white">
                    ...
                    <div class="row mt-3">
                        <!--타이틀-->
                        <div class="col-4 mt-3" ><span style="font-family:'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; color: rgb(46, 46, 46); font-weight:bold; font-size: 36px;">출고조회</span></div>
                        <div class="col"></div>
                        <!---네비게이터-->
                        <div class="col-7">
                            <div class="row">                                   
                                ...
                                <div class="col mt-5" style="display:inline;">     
                                    <ul style="list-style:none; margin-top: 10px; margin-bottom: 5px; margin-left: 100px; text-decoration-color: gray">
                                        <li>
                                            ...
                                            >
                                            <a style="color: gray; font-size: 14px" href="#">출고조회</a>
                    ...
                    <form action="./store_out_view_page.do" method="get">
    <div class="row">
    <div class="col-1"></div>
    <div class="col">
                              ...
                              <tbody>
                                 ...
                                 <tr>
                                    ...
                                    <td scope="col"><input placeholder="입고날짜를 입력해주세요."
                                       name="out_start_date" id="out_start_date" type="date"
                                       class="form-control form-control-sm"></td>
                                    <td colspan="2"><input placeholder="입고날짜를 입력해주세요."
                                       name="out_final_date" id="out_final_date" type="date"
    ...
    <option value=1 selected>출고번호순
				<option value=2>출고수량많은순
				<option value=3>출고수량적은순
				<option value=4>제품가격높은순
				<option value=5>제품가격낮은순
         </select></div>
   	</div>
   	</form>
		<div class="row"> <!-- 테이블 -->
					<div class="col">
						<table class="table table-hover shadow-sm p-3 mb-5 bg-white rounded" id="prolist">
							<thead class="shadow-none p-3 mb-5 bg-light rounded">
								<tr>
								<td>출고번호</td><td>제품번호</td><td>제품이름</td><td>출고수량</td>
								<td>제품가격</td><td>출고일자</td><td>출고처리사원</td>
								</tr>
							</thead>
							<tbody id="tbody">
							<c:forEach items="${store_out_list}" var="xxx">
									<tr>
										<td>${xxx.store_out_itemVo.store_out_item_no}</td>
										<td>${xxx.productVo.product_no}</td>
										<td>${xxx.productVo.product_name}</td>
										<td>${xxx.store_out_itemVo.out_item_qty}</td>	
										<td><fmt:formatNumber value="${xxx.productVo.product_price}" pattern="#,###"/></td>
										<td><fmt:formatDate value="${xxx.store_out_itemVo.out_item_date}" pattern="yy-MM-dd"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<fmt:formatDate value="${xxx.store_out_itemVo.out_item_date}" pattern="HH:mm:ss"/></td>									
					...
					<div class="col-8"> <!-- 페이지 버튼 -->
						<nav aria-label="Page navigation example">
						   <ul class="pagination">
							...
							<li class="page-item<c:if test="${beginPage-1 <= 0}"> disabled</c:if>"><a class="ttt page-link" href="./store_out_view_page.do?currPage=${beginPage-1 }&store_out_item_no=${param.store_out_item_no}&product_name=${param.product_name}&out_start_date=${param.out_start_date}&out_final_date=${param.out_final_date}&sort=${param.sort}">이전</a></li>
							  <c:forEach begin="${beginPage }" end="${endPage }" var="i">
								  <li class="page-item<c:if test="${currPage==i}"> active</c:if>"><a class="ttt page-link" href="./store_out_view_page.do?currPage=${i}&store_out_item_no=${param.store_out_item_no}&product_name=${param.product_name}&out_start_date=${param.out_start_date}&out_final_date=${param.out_final_date}&sort=${param.sort}">${i}</a></li>
							  </c:forEach>
							<li class="page-item<c:if test="${endPage+1 >= (totalCount-1)/10+1 }"> disabled</c:if>"><a class="ttt page-link" href="./store_out_view_page.do?currPage=${endPage+1 }&store_out_item_no=${param.store_out_item_no}&product_name=${param.product_name}&out_start_date=${param.out_start_date}&out_final_date=${param.out_final_date}&sort=${param.sort}">다음</a></li>
